...
(function () {
  ...
  function syncNewFolderButtons() {
    ...
    if (toolbarBtn) {
      ...
      toolbarBtn.title = disabled ? '全部模板不支持新建文件夹' : '新建文件夹';
    }
  }
  ...
  function moveFolderToParent(id, newParentId) {
    ...
    if (curParent === newParentId) return true;
    var siblingNames = getSiblingNames(newParentId);
    if (siblingNames.indexOf(node.name) !== -1) {
      UI.showToast('目标目录下已存在同名文件夹', 'error');
      return false;
    }
  ...
  if ($('moveFolderSubmit')) {
    $('moveFolderSubmit').addEventListener('click', function () {
      ...
      if (!moveFolderToParent(id, parentId)) return;
  ...
  $('folderSubmit').addEventListener('click', function () {
    ...
    if (mode === 'rename') {
      ...
      if (siblings.some(function (s) { return s.id !== targetId && s.name === name; })) {
        var errR = $('folderNameError');
        if (errR) errR.textContent = '同级已存在同名文件夹';
        if (item) item.classList.add('is-error');
        return;
      }
      node.name = name;
      ...
      renderAll();
      UI.showToast('文件夹已重命名', 'success');
      return;
    }

    var parentId = state.folderModal.parentId || SYSTEM_FOLDER.id;
    var siblingNames = getSiblingNames(parentId);
    if (siblingNames.indexOf(name) !== -1) {
      var err = $('folderNameError');
      if (err) err.textContent = '同级已存在同名文件夹';
      if (item) item.classList.add('is-error');
      return;
    }
    var neo = { id: 'f' + (++folderSeq), name: name, children: [] };
